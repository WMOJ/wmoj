  ...
  return (
    <div className="inline-flex items-center rounded-md overflow-hidden border border-border">
      {currentPage <= 1 ? (
        ...
        <button onClick={() => onPageChange(currentPage - 1)} className={`${base} hover:bg-surface-3 transition-colors`}>
          «
        </button>
      )}
      ...
      {pages.map((p, i) =>
        p === '...' ? (
          ...
          <button key={p} onClick={() => onPageChange(p as number)} className={`${base} hover:bg-surface-3 transition-colors`}>
            {p}
          </button>
        )
      )}
