...
export default function ManagerEditContestClient({ contest, initialProblems = [] }: { contest: ContestData; initialProblems?: SearchableProblem[] }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/manager/contests/${contest.id}`, {
        method: 'PATCH',
      ...
      if (res.ok) {
        ...
        setTimeout(() => router.push('/manager/contests/manage'), 2000);
        ...
        <div className="w-full space-y-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-5 max-w-4xl">
            ...
            <div className="flex gap-3">
              ...
              <Link href="/manager/contests/manage" className="h-10 px-5 bg-surface-2 text-foreground text-sm font-medium rounded-md hover:bg-surface-3 flex items-center">Cancel</Link>
            </div>
          </form>
        </div>
      </ManagerGuard>
